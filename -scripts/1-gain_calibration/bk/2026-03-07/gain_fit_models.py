...
def build_param_vector(model_name, n_peaks, init):
    ...
    amps = init.get('amplitudes', [max(10.0, float(np.sum(np.array([]))))] * n_peaks)
    for k in range(n_peaks):
        a0 = float(amps[k]) if k < len(amps) else 10.0
        names.append(f'A{k+1}')
        x0.append(a0)
        bds.append((0.0, max(a0 * 30.0, 100.0)))
...
def _stage1_single_peak(x_data, y_data, pos, gain_est):
    ...
    if np.sum(mask) < 4:
        return pos, gain_est * 0.12, max(float(y_data[np.argmin(np.abs(x_data - pos))]), 10.0)
    xi   = x_data[mask]
    yi   = y_data[mask].astype(float)
    A0   = float(np.max(yi))
    mu0  = float(xi[np.argmax(yi)])
    s0   = gain_est * 0.12
    bds1 = [(A0 * 0.05, A0 * 8), (mu0 - hw, mu0 + hw), (10.0, gain_est * 0.6)]
    ...
    return float(opt[1]), float(abs(opt[2])), float(max(opt[0], 0.0))
...
def fit_channel(x_data, y_data, n_peaks, peak_positions, gain_est, model_name,
    ...
    gain_wls, gain_err, icept, icept_err, r2, _ = linear_fit_gain(ns_s1, pm_s1, pe_s1)
