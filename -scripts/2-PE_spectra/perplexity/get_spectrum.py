...
try:
    from spectrum_utils import fit_calibration_peak, resolve_source, SOURCES
...
def process_single_rtraw(rtraw_file, calibration, dark_noise_pe, max_pe_per_channel=-1, pct_map=None):
...
# =============================================================================
# SYSTEMATIC FIT UNCERTAINTY
# Adapted from plot_energy_resolution.py::systematic_fit_study().
# Key change: dark_noise_pe (float) passed directly instead of a run-name
# string + static dict lookup — makes this self-contained in get_spectrum.
# =============================================================================
...
def systematic_fit_study(hist, method_name, dark_noise_pe):
    ...
    if len(results) < 2:
        printf(f"    WARNING: systematic study '{method_name}': "
    ...
    printf(f"    Systematic study '{method_name}': {len(results)}/9 fits OK  |  "
...
def _apply_error_budget(fit_result, sys_study):
    ...
    with the rest of get_spectrum, converting sys to fraction before combining.
    """
    out = dict(fit_result)
...
def get_spectrum(input_file, calib_file, out_file, merged_dir=None, file_type='rtraw',
                   ...
                   run_number=None,
    pct_file=None, source_pos=None, source_name=None):
    """
    ...
    for file_idx, input_file in enumerate(input_files, 1):
        ...
        if file_type == 'esd':
            ...
            result = process_single_rtraw(input_file, calibration, dark_noise_pe, max_pe_per_channel, pct_map=pct_map)
    ...
    if source_energy_mev is None:
        ...
        printf("=" * 60)
        result_pe_cont = fit_calibration_peak(
            h_PEcontin,   "Continous_PE", dark_noise_pe,
            expected_light_yield=4500.0, source_energy_mev=source_energy_mev, printf=printf)
        result_pe      = fit_calibration_peak(
            h_PEdiscrete, "Discrete_PE",  dark_noise_pe,
            expected_light_yield=4500.0, source_energy_mev=source_energy_mev, printf=printf)
        result_nhit    = fit_calibration_peak(
            h_nHit,       "nHit",         0.0,
            expected_light_yield=3800.0, source_energy_mev=source_energy_mev, printf=printf)

        # ── Error budget: statistical ⊕ systematic (fit-model dependence) ─────
        printf("")
        printf("=" * 60)
        printf("ERROR BUDGET (stat ⊕ systematic fit-model variation)")
        printf("=" * 60)

        if result_pe_cont:
            sys_cont       = systematic_fit_study(h_PEcontin,   "Continous_PE", dark_noise_pe)
            result_pe_cont = _apply_error_budget(result_pe_cont, sys_cont)
            printf(f"  Continuous PE — "
                   f"res: stat={result_pe_cont['resolution_error_stat']*100:.4f}%  "
                   f"sys={result_pe_cont['resolution_error_sys']*100:.4f}%  "
                   f"tot={result_pe_cont['resolution_error']*100:.4f}%")

        if result_pe:
            sys_disc  = systematic_fit_study(h_PEdiscrete, "Discrete_PE",  dark_noise_pe)
            result_pe = _apply_error_budget(result_pe, sys_disc)
            printf(f"  Discrete PE   — "
                   f"res: stat={result_pe['resolution_error_stat']*100:.4f}%  "
                   f"sys={result_pe['resolution_error_sys']*100:.4f}%  "
                   f"tot={result_pe['resolution_error']*100:.4f}%")

        if result_nhit:
            sys_nhit    = systematic_fit_study(h_nHit,       "nHit",         0.0)
            result_nhit = _apply_error_budget(result_nhit, sys_nhit)
            printf(f"  nHit          — "
                   f"res: stat={result_nhit['resolution_error_stat']*100:.4f}%  "
                   f"sys={result_nhit['resolution_error_sys']*100:.4f}%  "
                   f"tot={result_nhit['resolution_error']*100:.4f}%")
    ...
    # Continous PE
    if result_pe_cont:
        info_parts.append(f"RES_PEcontin={result_pe_cont['resolution']:.6f}")
        info_parts.append(f"RES_PEcontin_ERR={result_pe_cont['resolution_error']:.6f}")
        info_parts.append(f"RES_PEcontin_ERR_STAT={result_pe_cont.get('resolution_error_stat', result_pe_cont['resolution_error']):.6f}")
        info_parts.append(f"RES_PEcontin_ERR_SYS={result_pe_cont.get('resolution_error_sys', 0.0):.6f}")
        info_parts.append(f"MEAN_PEcontin={result_pe_cont['peak']:.6f}")
        info_parts.append(f"MEAN_PEcontin_ERR={result_pe_cont['peak_error']:.6f}")
        info_parts.append(f"MEAN_PEcontin_ERR_STAT={result_pe_cont.get('peak_error_stat', result_pe_cont['peak_error']):.6f}")
        info_parts.append(f"MEAN_PEcontin_ERR_SYS={result_pe_cont.get('peak_error_sys', 0.0):.6f}")
        info_parts.append(f"SIGMA_PEcontin={result_pe_cont['sigma']:.6f}")
        info_parts.append(f"SIGMA_PEcontin_ERR={result_pe_cont['sigma_error']:.6f}")
        info_parts.append(f"SIGMA_PEcontin_ERR_STAT={result_pe_cont.get('sigma_error_stat', result_pe_cont['sigma_error']):.6f}")
        info_parts.append(f"SIGMA_PEcontin_ERR_SYS={result_pe_cont.get('sigma_error_sys', 0.0):.6f}")
    else:
        info_parts.append("RES_PEcontin=-1;RES_PEcontin_ERR=0;RES_PEcontin_ERR_STAT=0;RES_PEcontin_ERR_SYS=0"
                          ";MEAN_PEcontin=-1;MEAN_PEcontin_ERR=0;MEAN_PEcontin_ERR_STAT=0;MEAN_PEcontin_ERR_SYS=0"
                          ";SIGMA_PEcontin=-1;SIGMA_PEcontin_ERR=0;SIGMA_PEcontin_ERR_STAT=0;SIGMA_PEcontin_ERR_SYS=0")

    # Discrete PE
    if result_pe:
        info_parts.append(f"RES_PE={result_pe['resolution']:.6f}")
        info_parts.append(f"RES_PE_ERR={result_pe['resolution_error']:.6f}")
        info_parts.append(f"RES_PE_ERR_STAT={result_pe.get('resolution_error_stat', result_pe['resolution_error']):.6f}")
        info_parts.append(f"RES_PE_ERR_SYS={result_pe.get('resolution_error_sys', 0.0):.6f}")
        info_parts.append(f"MEAN_PE={result_pe['peak']:.6f}")
        info_parts.append(f"MEAN_PE_ERR={result_pe['peak_error']:.6f}")
        info_parts.append(f"MEAN_PE_ERR_STAT={result_pe.get('peak_error_stat', result_pe['peak_error']):.6f}")
        info_parts.append(f"MEAN_PE_ERR_SYS={result_pe.get('peak_error_sys', 0.0):.6f}")
        info_parts.append(f"SIGMA_PE={result_pe['sigma']:.6f}")
        info_parts.append(f"SIGMA_PE_ERR={result_pe['sigma_error']:.6f}")
        info_parts.append(f"SIGMA_PE_ERR_STAT={result_pe.get('sigma_error_stat', result_pe['sigma_error']):.6f}")
        info_parts.append(f"SIGMA_PE_ERR_SYS={result_pe.get('sigma_error_sys', 0.0):.6f}")
    else:
        info_parts.append("RES_PE=-1;RES_PE_ERR=0;RES_PE_ERR_STAT=0;RES_PE_ERR_SYS=0"
                          ";MEAN_PE=-1;MEAN_PE_ERR=0;MEAN_PE_ERR_STAT=0;MEAN_PE_ERR_SYS=0"
                          ";SIGMA_PE=-1;SIGMA_PE_ERR=0;SIGMA_PE_ERR_STAT=0;SIGMA_PE_ERR_SYS=0")

    # nHit
    if result_nhit:
        info_parts.append(f"RES_NHIT={result_nhit['resolution']:.6f}")
        info_parts.append(f"RES_NHIT_ERR={result_nhit['resolution_error']:.6f}")
        info_parts.append(f"RES_NHIT_ERR_STAT={result_nhit.get('resolution_error_stat', result_nhit['resolution_error']):.6f}")
        info_parts.append(f"RES_NHIT_ERR_SYS={result_nhit.get('resolution_error_sys', 0.0):.6f}")
        info_parts.append(f"MEAN_NHIT={result_nhit['peak']:.6f}")
        info_parts.append(f"MEAN_NHIT_ERR={result_nhit['peak_error']:.6f}")
        info_parts.append(f"MEAN_NHIT_ERR_STAT={result_nhit.get('peak_error_stat', result_nhit['peak_error']):.6f}")
        info_parts.append(f"MEAN_NHIT_ERR_SYS={result_nhit.get('peak_error_sys', 0.0):.6f}")
        info_parts.append(f"SIGMA_NHIT={result_nhit['sigma']:.6f}")
        info_parts.append(f"SIGMA_NHIT_ERR={result_nhit['sigma_error']:.6f}")
        info_parts.append(f"SIGMA_NHIT_ERR_STAT={result_nhit.get('sigma_error_stat', result_nhit['sigma_error']):.6f}")
        info_parts.append(f"SIGMA_NHIT_ERR_SYS={result_nhit.get('sigma_error_sys', 0.0):.6f}")
    else:
        info_parts.append("RES_NHIT=-1;RES_NHIT_ERR=0;RES_NHIT_ERR_STAT=0;RES_NHIT_ERR_SYS=0"
                          ";MEAN_NHIT=-1;MEAN_NHIT_ERR=0;MEAN_NHIT_ERR_STAT=0;MEAN_NHIT_ERR_SYS=0"
                          ";SIGMA_NHIT=-1;SIGMA_NHIT_ERR=0;SIGMA_NHIT_ERR_STAT=0;SIGMA_NHIT_ERR_SYS=0")
    ...
    if result_nhit:
        print(
            f"{'nHit':<20}"
            f"{result_nhit['mean']:.1f} ± {result_nhit['mean_error']:.1f:<15}"
