...
{
    std::cout<<"////////////"<<std::endl;
    std::cout<<"readRootGeoFile start"<<std::endl;
    std::cout<<"////////////"<<std::endl;
    Int_t olderrlevel = gErrorIgnoreLevel;
    gErrorIgnoreLevel = kWarning;

    // load from InputSvc
    SniperPtr<RootInputSvc> inputsvc(getParent(), "InputSvc");
    if (inputsvc.valid()) {
        TObject* obj = 0;
        inputsvc->getObj(obj, "TaoGeom", "/Event/Sim");
        LogInfo << "load geometry from inputsvc: " << obj << std::endl;
        if (obj) {
            m_geom_mgr = dynamic_cast<TGeoManager*>(obj);
        }
    } else {
    ...
    m_geom_mgr = TGeoManager::Import(m_input_root_file.c_str(),
                                    "TaoGeom");
    if (m_geom_mgr) {
        LogInfo << "load geometry successfully" << std::endl;
    }
    std::cout<<"////////////"<<std::endl;
    std::cout<<"readRootGeoFile end"<<std::endl;
    std::cout<<"////////////"<<std::endl;
